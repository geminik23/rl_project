...
#=================================================================NETWORK
# New newtwork
# architecture consists of two separate estimators, state-value function and action-advantage function.
# split into two outputs
# For this network, I won't use the Sequential
...
class DuelingNetwork(nn.Module):
    def __init__(self, input_dim, output_dim):
        ...
        self._input = nn.Linear(input_dim, 32)
        self._hidden_1 = nn.Linear(32, 32)
        self._hidden_2 = nn.Linear(32, 32)
        self._value = nn.Linear(32, 1)
        self._advantage = nn.Linear(32, output_dim)

    def forward(self, state):
        x = state
        x = self._activation(self._input(x))
        x = self._activation(self._hidden_1(x))
        x = self._activation(self._hidden_2(x))
        a = self._advantage(x) 
